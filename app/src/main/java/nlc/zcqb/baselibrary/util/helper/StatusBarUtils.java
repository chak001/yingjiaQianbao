...
//                attributes.flags |= flagTranslucentStatus;
     ...
     * @param isBlack
     */
    public static void changeStatusTextColor(Activity  activity,boolean isBlack){
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.M) {
            int flag=  activity.getWindow().getDecorView().getSystemUiVisibility();
            if (isBlack) {
                flag =flag | View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR;
                activity.getWindow().getDecorView().setSystemUiVisibility(flag);//设置状态栏黑色字体
            }else {
                flag =flag | View.SYSTEM_UI_FLAG_VISIBLE;
                activity.getWindow().getDecorView().setSystemUiVisibility(flag);//恢复状态栏白色字体
            }
        }
    }
}
